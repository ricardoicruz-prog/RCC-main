...
Reading `/tmp/auto/scrape.json` + `profiles/<slug>/client-profile.yaml`, Claude writes Prospeo filters to `/tmp/auto/filters.json`:
...
Claude MUST use exact Prospeo industry names from `.claude/skills/icp-onboarding/references/prospeo-industries.md`.
...
Write to `profiles/<slug>/experiments/<YYYY-MM-DD>-<target>.json`:
...
Everything the skill needs lives under `profiles/<slug>/`:
